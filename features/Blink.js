...
new KeyBind("Stop Recording - Blink", Keyboard.KEY_NONE, "JerryAddons").registerKeyPress(() => toggleRecording(false))
new KeyBind("Charge Packets - Blink", Keyboard.KEY_NONE, "JerryAddons").registerKeyPress(() => toggleCharge())
...
register("command", (...args) => {
    ...
    if (!FileLib.exists("JerryAddons/data/Blink/", routeName + ".json")) return sendMsg(`&7Route doesn't exist`)
...
function toggleRecording(state = !selectedRouteName) {
    ...
    if (packetLogToggle) {
        FileLib.delete("JerryAddons/data/Blink/", selectedRouteName + ".json")
        FileLib.append("JerryAddons/data/Blink/", selectedRouteName + ".json", `Speed when this route was recorded: ${((Player.getPlayer().field_71075_bZ.func_75094_b()) * 1000).toFixed(0)}`)
...
register("command", (...args) => {
    ...
    const dir = new File("./config/ChatTriggers/modules/JerryAddons/data/Blink")
    if (!dir.exists()) dir.mkdirs()
    if (FileLib.exists("JerryAddons/data/Blink", fileName + ".json")) return sendMsg(`&7Route already exists`)
    FileLib.write("JerryAddons/data/Blink", fileName + ".json", "")
    sendMsg(`&7Created ${fileName}.json`)
}).setName("blinkcreate")

register("command", (...args) => {
    const fileName = args.join(" ")
    if (!FileLib.exists("JerryAddons/data/Blink", fileName + ".json")) return sendMsg(`&7Route doesn't exist`)
    FileLib.delete("JerryAddons/data/Blink/", fileName + ".json")
...
function updateRoutes() {
    routes = new File("./config/ChatTriggers/modules/JerryAddons/data/Blink").list()
    parsedFiles = routes.map(fileName => parseFile(fileName))
}

function parseFile(fileName) {
    try {
        const packets = FileLib.read("JerryAddons/data/Blink", fileName).split("\n").map(str => str.split(", "))
        packets.shift()
        return packets
    } catch (e) {
        return null
